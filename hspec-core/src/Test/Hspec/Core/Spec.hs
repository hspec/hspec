...
limit :: Seconds -> SpecWith a -> SpecWith a
limit s = mapSpecItem_ $ \i -> i {itemMaxTime = SetTo s}

-- | Ensure there is no time limit on the given tests.
noLimit :: SpecWith a -> SpecWith a
noLimit = mapSpecItem_ $ \i -> i {itemMaxTime = UnSet}
