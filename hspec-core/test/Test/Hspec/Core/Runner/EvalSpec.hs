...
spec = do
  let mkCfg f = do
        ...
        let cfg = H.defaultConfig{H.configFormatter = Just $ pure traceF}
...
traceProperties mkCfg = do
  ...
  traceProperty mkCfg "group starts before children" $ \_model ->
    fst . flip foldl' (True, []) (\(allGood, groups) x -> case x of
      ...
      ExampleStarted (path,_) -> ((path `elem` groups) && allGood, groups)
      _ -> (allGood, groups))

  it "parallel examples start immediately" $ do
    (cfg, getLines) <- mkCfg
    m <- newEmptyMVar
    H.hspecWith cfg $ do
          H.describe "sequential" $ do
            H.it "waits" $ takeMVar m >> sleep 10
            H.it "1" True
          H.describe "parallel" $ H.parallel $ do
            H.it "2" True
            H.it "3" (putMVar m ())
    tr <- getLines
    tr `shouldStartWith` [ExampleGroupStarted [] "sequential", ExampleStarted (["sequential"], "waits")]
    tr `shouldEndWith`
      [ ExampleSucceeded(["sequential"], "waits") ""
      , ExampleStarted (["sequential"], "1")
      , ExampleSucceeded (["sequential"], "1") ""
      , ExampleGroupDone [] "sequential"]
...
data FormatterLine
  ...
  | ExampleGroupDone [String] String
  | ExampleProgress H.Path (Int,Int)
  | ExampleStarted H.Path
...
traceFormatterIO = do
  ...
  let push x = liftIO $ modifyIORef' ref (x :)
      f = silent
        ...
        , exampleStarted      = push . ExampleStarted
...
modelSequentialTrace = go [] (1::Int) where
  ...
  go path idx (Describe {steps}) =
    ...
    [ExampleGroupDone path name]
    where
      name = "Group " ++ show idx
  go' path idx (Success _) =
    [ ExampleStarted (path, "Example " ++ show idx)
