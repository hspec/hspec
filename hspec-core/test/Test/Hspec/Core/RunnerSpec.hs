...
import           Test.Hspec.Core.Runner (ReportProgress(..), OutputColor(..))
...
spec = do
  ...
  describe "colorOutputSupported" $ do
      ...
      context "with GITHUB_ACTIONS=true" $ do
        it "enable color output" $ do
          withEnvironment [("GITHUB_ACTIONS", "true")] $ do
            H.colorOutputSupported H.ColorAuto isTerminalDevice `shouldReturn` ColorEnabled ProgressDisabled
    ...
    context "with a terminal device" $ do
      ...
      it "enable color output" $ do
        H.colorOutputSupported H.ColorAuto isTerminalDevice `shouldReturn` ColorEnabled ProgressEnabled
