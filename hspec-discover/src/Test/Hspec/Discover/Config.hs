...
, configModulePrefix :: Maybe String
} deriving (Eq, Show)

defaultConfig :: Config
defaultConfig = Config False Nothing False Nothing Nothing
...
options = [
  ...
  , Option [] ["module-prefix"] (ReqArg (\s c -> c {configModulePrefix = Just s}) "PREFIX") ""
  , Option [] ["no-main"] (NoArg $ \c   -> c {configNoMain = True}) ""
  ]

usage :: String -> String
usage prog = "\nUsage: " ++ prog ++ " SRC CUR DST [--module-name=NAME] [--module-prefix=PREFIX]\n"
